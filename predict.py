...
my_model.load_model("my_model.bin")

zip_code_df = pd.read_csv(
    "https://raw.githubusercontent.com/wlifferth/build-an-ml-web-app/main/median_income_by_zip_code.csv"
)
...
def predict(data):
    ...
    prediction_dataframe["homeType"] = prediction_dataframe["homeType"].astype(
        pd.CategoricalDtype(
            categories=["SINGLE_FAMILY", "CONDO", "TOWNHOUSE", "MULTI_FAMILY"]
        )
    )
    prediction_dataframe["state"] = prediction_dataframe["state"].astype(
        pd.CategoricalDtype(categories=states_dict.keys())
    )

    prediction_dataframe = pd.get_dummies(prediction_dataframe, columns=["homeType"])

    prediction_dataframe = pd.merge(
    ...
    prediction_dataframe["median_income"].fillna(
        prediction_dataframe["median_income"].mean(), inplace=True
    )
